      ...
      <div class="col-md-12">
        <div class="row q-mx-sm q-mt-sm">
          <div class="col-6 offset-3 q-mt-md text-cyan-4 text-h6">
            <div class="row justify-center">
            {{ countMatieres }} matières premières en attente de commande
            </div>
          </div>
            ...
            <div class="row justify-start">
              <div class="col-2 q-pa-sm" v-for="matiere in filteredMatieres" :key="matiere.id">
                <q-card bordered class="card-maxi justify-center items-center text-center relative" :class="{ 'bd-red-4': !matiere.stockee}">
                  ...
                  <q-btn-group class="absolute-bottom-right">
                    <q-btn
                      class="hover-effect"
...
export default {
  ...
  computed: {
    ...
    countMatieres() {
      return this.filteredMatieresCommande.length;
    },

    filteredMatieresCommande() {
      return this.allMatieres.filter(matiere => matiere.cde === true);
    },

    filteredMatieres() {
      if (this.searchQuery) {
        return this.filteredMatieresCommande.filter(matiere =>
            matiere.nom.toLowerCase().startsWith(this.searchQuery.toLowerCase())
        );
      }
      return this.filteredMatieresCommande;
    },
  ...
  methods: {
    ...mapActions('matieresPremieres', ['loadMatieresPremieres', 'toggleMenu', 'toggleCde', 'toggleLivraison', 'toggleInfo']),
